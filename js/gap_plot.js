...
class GapPlot {
    ...
    constructor(data, updateCountry, updateYear, activeYear) {
        ...
        this.data = data;
        this.countryMap = {}
        this.data['population'].forEach((d) => {
            this.countryMap[d.geo] = d.region
        })
        console.log(data.population);
        this.updateCountry = updateCountry;
        this.drawPlot();
        this.drawYearBar();
        this.xValue = 'population'
        this.yValue = 'population'
        this.cValue = 'population'
        this.updatePlot(this.activeYear,this.xValue,this.yValue, this.cValue);
        this.drawDropDown()
        console.log(this)
    ...
    drawPlot() {
        ...
        this.svgContainer =  d3.select('#chart-view').select('.plot-svg')
        let svgGroup = this.svgContainer.append('g').classed('wrapper-group', true)
        this.svgContainer.append('g').attr('class', 'circle-group')
        //YOUR CODE HERE  
        svgGroup.append("g")
            .classed("x-axis", true)
    ...
    updatePlot(activeYear, xIndicator, yIndicator, circleSizeIndicator) {
        ...
        */
        ...
        let circleSizer = function(d) {
            let cScale = d3.scaleSqrt().range([3, 20]).domain([this.minSize, this.maxSize]);
        ...
        this.svgContainer.select('.axis-label-x').text(xIndicator)
        this.svgContainer.select('.axis-label-y').text(yIndicator)
        this.svgContainer.select('.year-label').text(activeYear)
        
        console.log({activeYear, xIndicator, yIndicator, circleSizeIndicator}, this.svgContainer)

        console.log(this)
        console.log(this.data[xIndicator])
        ...
        let xScale = d3.scaleLinear()
            .domain(d3.extent(xItems))
            .range([
                this.margin.left,
                this.width - this.margin.right
            ])
        let yScale = d3.scaleLinear()
            .domain(d3.extent(yItems))
            .range([
                this.height - this.margin.bottom,
                this.margin.top
            ])
        let xGroup = this.svgContainer.select('.x-axis').call(d3.axisBottom(xScale))
        let yGroup = this.svgContainer.select('.y-axis').call(d3.axisLeft(yScale))

        let circleGroup = this.svgContainer.select('.circle-group')
        let dataMap = {}
        ...
        Object.keys(dataMap).forEach((k) => {
            if (dataMap[k].x && dataMap[k].y && dataMap[k].value) {
                data.push({
                    country: k,
                    x: dataMap[k].x,
                    y: dataMap[k].y,
                    value: dataMap[k].value,
                    geo: dataMap[k].geo
                })
            }
        })
        this.minSize = d3.extent(data, (d) => {
            return d.value
        })[0]
        this.maxSize = d3.extent(data, (d) => {
            return d.value
        })[1]
        let cScale = d3.scaleSqrt().range([3, 20]).domain([this.minSize, this.maxSize]);
        circleGroup.selectAll('*').remove()
        let circles = circleGroup.selectAll('circle').data(data).enter()
            .append('circle')
            .attr('cx', (d) => {
                return xScale(d.x)
            })
            ...
            .attr('class', (d) => {
                return this.countryMap[d.geo]
            })
        circles
            .on('click', (d) => {
                this.updateHighlightClick(d.geo.toUpperCase())
            })
        circles.append('title')
            .text((d) => {
                return d.country
            })
        this.drawLegend(this.minSize, this.maxSize)
    }
    ...
    drawYearBar() {
        ...
        /* ******* TODO: PART 3 *******
        You will need to call the() function passed from script.js in your activeYear slider
        */
        let that = this;
    ...
    drawLegend(min, max) {
        ...
        let svg = d3.select('.circle-legend').select('svg')
        ...
        circleGroup.attr('transform', (d, i) => 'translate(' + ((i * (4 * scale(d))) + 10) + ', 25)');
    ...
    updateHighlightClick(activeCountry, flag = true) {
        ...
        */
        ...
        if (flag) {
            window.worldMap.updateHighlightClick(activeCountry, false)
        }
        window.infoBox.updateTextDescription(activeCountry, this.activeYear)
    }
    ...
    clearHighlight() {
        ...
        window.worldMap.clearHighlight()
    }
